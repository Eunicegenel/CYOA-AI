...
const MANGA_OUTPUT_DIR = path.join(
  __dirname,
  "../../data/pokemon/raw/lore/manga"
);

const PAGES_DIR = path.join(MANGA_OUTPUT_DIR, "pages");

const INDEX_PATH = path.join(MANGA_OUTPUT_DIR, "_manga_index.json");
const CATEGORY_INDEX_PATH = path.join(
  MANGA_OUTPUT_DIR,
  "_manga_categories.json"
);
const FAILED_PATH = path.join(MANGA_OUTPUT_DIR, "_manga_failed.json");
const STATE_PATH = path.join(MANGA_OUTPUT_DIR, "_manga_crawl_state.json");

const BULBAPEDIA_API_URL =
  "https://bulbapedia.bulbagarden.net/w/api.php";

const ROOT_CATEGORY = "Category:Manga";
...
function ensureFolders() {
  fs.mkdirSync(MANGA_OUTPUT_DIR, { recursive: true });
...
async function fetchJson(url) {
  const response = await fetch(url, {
    headers: {
      "User-Agent":
        "CYOA-AI Pokemon Manga Lore Downloader/1.0 (local development project)",
    },
  });
...
async function crawlMangaCategoryTree() {
  console.log("Discovering every page under Category:Manga...\n");
  ...
  console.log(`Total manga categories: ${categories.length}`);
  console.log(`Total manga pages discovered: ${discoveredPages.length}\n`);
...
async function fetchBulbapediaPage(title) {
  ...
  return {
    ...
    canon: "manga",
    loreScope: "Category:Manga recursive crawl",
    wikitext,
  };
}

async function downloadMangaPages(pages) {
  console.log("Downloading manga page wikitext...\n");
  ...
  console.log("\nManga page download pass complete.");
...
async function runMangaLoreDownload() {
  ensureFolders();

  console.log("Starting full Bulbapedia Manga lore crawl...\n");

  const discoveredPages = await crawlMangaCategoryTree();
  await downloadMangaPages(discoveredPages);

  console.log("\nFull Manga lore crawl finished.");
}

runMangaLoreDownload().catch((error) => {
  console.error("\nManga lore download crashed:");
  console.error(error);
  process.exit(1);
});
